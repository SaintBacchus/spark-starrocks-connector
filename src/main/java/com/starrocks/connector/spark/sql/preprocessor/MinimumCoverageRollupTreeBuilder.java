...
public class MinimumCoverageRollupTreeBuilder implements RollupTreeBuilder {
    public RollupTreeNode build(EtlJobConfig.EtlTable tableMeta) {
        List<EtlJobConfig.EtlIndex> indexes = tableMeta.getIndexes();
        ...
        List<EtlJobConfig.EtlColumn> baseIndexColumns = baseIndex.getColumns();
        ...
        for (EtlJobConfig.EtlColumn columnMeta : baseIndexColumns) {
            if (columnMeta.isKey()) {
                baseKeyColumns.add(columnMeta.getColumnName());
            } else {
                baseValueColumns.add(columnMeta.getColumnName());
            }
        }
        ...
        root.indexId = baseIndex.getIndexId();
        ...
        for (int i = 0; i < indexMetas.size(); ++i) {
            ...
            for (EtlJobConfig.EtlColumn column : indexMetas.get(i).getColumns()) {
                if (column.isKey()) {
                    keyColumns.add(column.getColumnName());
                } else {
                    valueColumns.add(column.getColumnName());
                }
            }
    ...
    private boolean insertIndex(RollupTreeNode root, EtlJobConfig.EtlIndex indexMeta,
        ...
        if (allColumnNames.containsAll(keyColumns) && allColumnNames.containsAll(valueColumns)) {
            ...
            newChild.indexId = indexMeta.getIndexId();
