...
public abstract class SparkRDDAggregator<T> implements Serializable {
    ...
    public static SparkRDDAggregator buildAggregator(EtlJobConfig.EtlColumn column) throws SparkWriteSDKException {
        String aggType = StringUtils.lowerCase(column.getAggregationType());
        String columnType = StringUtils.lowerCase(column.getColumnType().getName());
        switch (aggType) {
            case "bitmap_union":
...
class BitmapUnionAggregator extends SparkRDDAggregator<BitmapValue> {
    ...
    BitmapValue init(Object value) {
        ...
        try {
            ...
            throw new RuntimeException("build bitmap value failed, " + e.getMessage(), e);
        }
    }
...
class HllUnionAggregator extends SparkRDDAggregator<Hll> {
    ...
    Hll init(Object value) {
        ...
        try {
            ...
            throw new RuntimeException("build hll value failed, " + e.getMessage(), e);
        }
    }
    ...
    public int compare(List<Object> keyArray1, List<Object> keyArray2) {
        ...
        for (int i = 0; i < keyArray1.size(); i++) {
            ...
            if (key1 instanceof Comparable && key2 instanceof Comparable) {
                ...
                throw new RuntimeException(String.format("uncomparable column type %s", key1.getClass()));
            }
            if (cmp != 0) {
                return cmp;
            }
